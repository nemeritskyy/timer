...
(function (global, factory) {
	...
	function isValid(m) {
		let flags = null;
		let parsedParts = false;
		let isNowValid = m._d && !isNaN(m._d.getTime());
		if (isNowValid) {
	...
	const match1to2HasZero = /^([1-9]\d|\d)/; //           0-99
	let regexes;
	...
	addParseToken(['N', 'NN', 'NNN', 'NNNN', 'NNNNN'], (input, array, config, token) => {
	...
	const valueOf$1 = asMilliseconds;
	...
	function toISOString$1() {
		...
		return `${totalSign}P${years ? `${ymSign + years}Y` : ''}${
			months ? `${ymSign + months}M` : ''
		}${days ? `${daysSign + days}D` : ''}${hours || minutes || seconds ? 'T' : ''}${
			hours ? `${hmsSign + hours}H` : ''
		}${minutes ? `${hmsSign + minutes}M` : ''}${seconds ? `${hmsSign + s}S` : ''}`;
	}
